...
ENetClientObject::EnetClientObject() {
    ...
    m_address.host = "127.0.0.1";
    m_address.port = DEFAULT_PORT;
    
    m_client = enet_host_create(nullptr, 0, 2, 0, 0);
    enet_address_set_host(m_client, &m_address);
    
    m_peer = enet_host_connect(m_server, &address, 1, 0);
    ...
    while(enet_host_service(m_client, &m_event, 100) > 0) {
        switch(m_event.type) {
            case ENET_EVENT_TYPE_RECEIVE : 
                printf("A packet from %x:%u was received, containing %d bytes.\n",
